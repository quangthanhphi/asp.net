...
public class ProductCateViewComponent : ViewComponent
{
    ...
    public ProductCateViewComponent(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<IViewComponentResult> InvokeAsync(int productCategoryId)
    {
        ...
        // Extract ProductCategoryName from the first item in the list (assuming there's at least one item)
        string productCategoryName = productViewModels.FirstOrDefault()?.ProductCategoryName;
